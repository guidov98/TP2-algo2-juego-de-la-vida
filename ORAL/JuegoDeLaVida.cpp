...
void JuegoDeLaVida::iniciarConfiguracionTres(){
	...
	for (int i=0; i<5; i++){
		for (int j=0; j<2; j++){
			nacimientos +=this->tablero->revivirCelulaPos(i,	0,	j*3,	50,	100,25);
			nacimientos +=this->tablero->revivirCelulaPos(i,	6,	j*3,	200,5,	150);
			nacimientos +=this->tablero->revivirCelulaPos(0,	i+1,j*3,	5,	5,	150);
			nacimientos +=this->tablero->revivirCelulaPos(4,	i+1,j*3,	5,	5,	5);
		}
	}
	for (int i=1; i<3; i++){
		nacimientos +=this->tablero->revivirCelulaPos(0,0,i,0,0,0);
		nacimientos +=this->tablero->revivirCelulaPos(4,0,i,0,0,0);
		nacimientos +=this->tablero->revivirCelulaPos(0,6,i,0,0,0);
		nacimientos +=this->tablero->revivirCelulaPos(4,6,i,0,0,0);
	}
	this->nacimientosTotales = nacimientos;
...
void JuegoDeLaVida::iniciarConfiguracionManual(){
	...
	std::cout << "alrededor de una celula viva para que siga viva (" << x2user << "-26): ";
	x3user = Funciones::pedirInput(x2user, MAXIMOS_VECINOS);
	std::cout << std::endl << std::endl;
...
int JuegoDeLaVida::getCantidadCelulasVivas(){
	return this->tablero->getCantidadCelulas();
}
...
void JuegoDeLaVida::ejecutarTurno(){
	...
	this->turnoActual++;
	float promedioNacimientos = (float)(this->nacimientosTotales) / (float)(this->turnoActual);
	float promedioMuertes = (float)(this->muertesTotales) / (float)(this->turnoActual);
	this->mostrarTablero();
	...
	this->tablero->actualizarComportamientoCeldas();
}
